...
- **No sample widget** — `manifest.json` ships with an empty `widgets[]` and there is no `src/widgets/` folder. You add widgets yourself.
...
npm run build    # tsc + vite build + youtrack-app validate dist
...
| Sample widget | none (empty `widgets[]`) | pre-scaffolded |
